...
This package contains only the current runtime source and essential operation
documents. Legacy backups, old version changelogs, test fixtures, and unused
root compatibility loaders were removed.
...
Do not restore root `style.css` or `script.js`; the current `index.html` does
not use them.
